/**
 * scripts/accept/sign-n29-acceptance.ts —— N29 refix D-5：为验收 envelope 盖上 CI/发布密钥签名。
 *
 * 输入是 `scripts/accept/accept-n29-minimal-intake.ts --output` 生成的**未签名** envelope JSON；
 ...
 *
 * 用法：
 *   npx tsx scripts/accept/sign-n29-acceptance.ts \
...
async function main(): Promise<void> {
  ...
  if (!inputPath || !outputPath || !privateKeyPath) {
    process.stderr.write(
      "用法: npx tsx scripts/accept/sign-n29-acceptance.ts --input <envelope.json> --output <signed.json>"
