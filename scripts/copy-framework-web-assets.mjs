 ...
 *
 * 约束：只允许 index.html / styles.css / app.js 三个文件；拒绝符号链接与未知扩展名；
 * 复制前只删除该目标目录，不触碰其他产物。
 */
...
if (copied.length !== ALLOWED_FILES.size || copied.some((f) => !ALLOWED_FILES.has(f))) {
  throw new Error(`operator-console public directory must contain exactly index.html, styles.css, app.js; got: ${copied.join(", ")}`);
}
