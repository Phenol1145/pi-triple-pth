/**
 * scripts/eval/eval-n29-minimal-intake.ts —— N29 最小可信摄入内环 provisional evaluator（纯判据）。
 *
 * 定位（计划 §5 Task 7 Step 1/2/5）：
 *  - 本文件**只做判据**：解析 Vitest JSON 报告与聚焦套件写出的分母台账，导出
 *    正向分母核对、负向 sentinel 派生与 skip manifest 生成；不自己下 GO 结论。
 *  - 唯一权威结论由 `scripts/accept/accept-n29-minimal-intake.ts` 给出（它把这里的判据与
 *    typecheck / full regression / lint / build 门禁和 evaluated commit 绑在一个 envelope 里）。
 *  - CLI 模式（`npx tsx scripts/eval/eval-n29-minimal-intake.ts`）跑一次最小内环集成测试并核对
