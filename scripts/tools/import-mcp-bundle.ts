 ...
 *   DATABASE_URL=… npx tsx scripts/tools/import-mcp-bundle.ts <bundle.json>          # 落库（draft 提案）
 *   DATABASE_URL=… npx tsx scripts/tools/import-mcp-bundle.ts <bundle.json> --dry-run # 只校验+打印
 *
...
if (!fileArg) {
  console.error("用法: DATABASE_URL=… npx tsx scripts/tools/import-mcp-bundle.ts <bundle.json> [--dry-run]");
  process.exit(1);
}
