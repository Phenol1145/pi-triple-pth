/**
 * scripts/gen/build-discipline-catalog.ts — K0 Discipline Catalog 数据生成器。
 *
...
function main(): void {
  ...
  if (checkMode) {
    for (const { file, content } of generatedFiles) {
      ...
      if (disk !== content) {
        fail(`--check 失败：${relative(ROOT, file)} 与重新解析内容不一致——请运行 npx tsx scripts/gen/build-discipline-catalog.ts 重新生成`);
      }
    }
    console.log(
