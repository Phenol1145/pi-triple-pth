...
export const PTC_TOOL_DEFS: PtcToolDef[] = [  { name: "python.run", anchor: "python 程序——python 生态/数据计算的多语句执行", whenToUse: "需要 python 库、多语句/循环、_result 值回传；ts 能做的用 ts（ts 程序内可 await python 能力）", effect: "多语句执行，_result 值回传", properties: {"code":{"type":"string","description":"python 程序（多语句；_result = 值 作为结果）"},"mode":{"type":"string","enum":["default","value-only","errors-only","quiet"]}}, required: ["code"] },
  ...
  { name: "asp.cd", anchor: "空间切换", whenToUse: "语言代码须在对应动作空间执行；done 仅元空间可用", effect: "切换当前空间（工具面随之切换）", properties: {"space":{"type":"string","description":"目标空间 id（meta/ts/python/bash/dev/write 或自定义注册空间）"}}, required: ["space"] },
  { name: "asp.create", anchor: "空间生成（治理）", whenToUse: "父空间声明 allowChildren 且填 childParams 表单（能力面收窄+记忆域分配）；meta 禁建", effect: "注册子空间，可 asp.cd 进入", properties: {"id":{"type":"string","description":"新空间 id（小写字母数字连字符 ≤32）"},"execTool":{"type":"string","description":"子空间语言执行工具名（能力面收窄——须为已注册语言族：ts/python/bash/dev/write）"},"memoryScope":{"type":"string","description":"记忆域标注（子空间记忆域名——缺省继承父空间；实际可见性过滤按空间 id 树）"},"extraTools":{"type":"string","description":"工具族收窄（逗号分隔——父空间 extraTools 的子集）"},"skeleton":{"type":"string","description":"语言骨架摘要（索引/prompt 用）"},"description":{"type":"string","description":"子空间说明（必填）"}}, required: ["id","execTool","memoryScope","description"] },
  { name: "asp.destroy", anchor: "空间注销（治理）", whenToUse: "仅子空间的父空间内可注销自己的子空间；内置空间保护不变", effect: "子空间注销", properties: {"id":{"type":"string","description":"要注销的空间 id"}}, required: ["id"] },
  { name: "asp.index", anchor: "空间地图——新任务/切空间前先看", whenToUse: "避免盲目 asp_cd 往返", effect: "空间树 + 当前空间可达函数与数据", properties: {"mode":{"type":"string","enum":["by-package","by-type"],"description":"聚合模式（缺省 by-package）"},"space":{"type":"string","description":"目标空间 id（缺省当前空间）"}}, required: [] },
