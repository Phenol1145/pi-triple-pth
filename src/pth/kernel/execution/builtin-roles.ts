...
export const DEFAULT_ROLES: WorkerRole[] = [
  ...
  { id: "coder", tags: ["coding", "write-code", "snippet"], prompt: "你是代码编写者——负责编写代码实现、片段、脚本。只写代码——不调试、不测试、不写文档（调试交给 debug 工具、测试交给 tester、文档交给 writer）。",
    ...
    output: "implementation", defaultReads: ["context", "plan"], acceptanceRole: "writer",
    parent: "developer", generation: 4, differentiation: "纯代码编写任务诱导——写代码不调试不测试——从 developer 收窄出代码编写专精" },
  { id: "scout", tags: ["recon", "investigate"], prompt: "你是侦查者——负责信息收集、代码侦察、环境探查。",
    ...
    parent: "explorer", generation: 3, differentiation: "侦察类任务诱导——快速信息收集不需要深推理——thinking low 特化换速度" },
  { id: "spider", tags: ["crawl", "scrape", "fetch"], prompt: "你是信息抓取者——负责网页抓取、结构化信息采集、多源数据聚合。通过 web 能力与 ext.use(agent-reach) 抓取网页/平台内容，抽取结构化数据，压缩为 context 交接下游分析。",
    ...
    parent: "explorer", generation: 3, differentiation: "抓取类任务诱导——网页/平台内容结构化采集需要 web+ext(agent-reach) 能力——从 explorer 分出爬取专精" },
  { id: "memory-keeper", tags: ["memory", "organize"], prompt: "你是记忆维护者——负责记忆整理、知识沉淀、索引维护。",
    ...
    parent: "researcher", generation: 3, differentiation: "记忆维护类任务诱导——知识沉淀/索引维护围绕 memory 能力收窄——记忆属于知识一类，归研究族专项维护（2026-08-14 用户裁决）" },
  ...
  { id: "writer", tags: ["write", "doc", "story", "tutorial", "article"], prompt: "你是写作者——负责文档编写、小说创作、教程撰写、内容生产。工作流：大纲→草稿→修订→定稿——文档写任务工作区（asp.cd(\"write\") → write.create/edit/read/list/save + write.section 章节组织）。不写代码不调试——文档不编译。",
    ...
    parent: "executor", generation: 3, differentiation: "编写类任务诱导——文档创作不需要执行核——能力收窄至读写+记忆，工具面引导 write 空间" },
];
