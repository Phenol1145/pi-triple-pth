...
import { setRuntimeCatalog, loadDefaultRoleSets } from "../catalog/index.js";
...
export interface BuildPthHostOptions {
  ...
  /** 测试注入；缺省从 ~/.pi-triple/services.json（不存在 = 空）加载 */
...
export async function buildPthHost(
  ...
  setProfessionalRoles(loadDefaultRoleSets().professionalRoles);
  ...
  const serviceRegistry = options.serviceRegistry ?? loadServiceRegistry();
