/**
 * execution/network/artifact-store.ts — task-scoped ArtifactStore adapter。
 *
 * V1 使用内存实现：artifact 内容只在 gateway（通常 per task）生命周期内存在，
 * PG 只保存 manifest/引用。`ArtifactRefV1` 从 V1 起稳定，未来可替换为 object store。
 */
...
export class InMemoryArtifactStore implements ArtifactStore {
  ...
  async get(ref: ArtifactRefV1): Promise<StoredArtifact> {
    ...
    if (!stored) {
      throw createNetworkExecuteError("NET_ARTIFACT_MISMATCH", `artifact ${ref.artifactId} 不存在或已过期`);
    }
