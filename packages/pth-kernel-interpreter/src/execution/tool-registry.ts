 ...
 * 该函数依赖 execution 的 knownRoleById / toolsToSchema，因此由 kernel-execution
 * 注入到 ExtContext.toolFaceBudgetCheck；本包只声明类型，避免 interpreter → execution 反向依赖。
 */
export interface ToolFaceBudgetCheckResult {
  ok: boolean;
  over: Array<{ role: string; face: number; projected: number }>;
}
export type ToolFaceBudgetCheck = (
  store: ToolRegStoreLike,
  candidate: ToolRegSpec,
  budget: number,
  opts?: { tenantId?: string },
) => Promise<ToolFaceBudgetCheckResult>;
