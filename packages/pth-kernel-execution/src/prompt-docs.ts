...
export const PROJECT_DIR_DUTY: Record<string, string> = {
  ...
  "src/pth/runner": "任务执行装配——exec-modes（tool-call/asp/ptc/pulse）/ agent-task-runner / runner-config",
  "src/pth/tasking": "任务池——claim/run/commit、delegation、penetration、side-effect outbox",
  "src/pth/interaction": "Human Interaction——intent-resolver / task-draft / presentation / PG 仓库",
  ...
  "src/pth/impls": "具体实现——kernels（ts-interpreter/capability）/ roles / 默认角色",
  "src/pth/services": "服务层——常驻服务接线",
  "src/pth/shared": "应用共享——pg-queryable 等",
  "src/pth/tools": "工具面——tool 注册与执行",
  "src/pth/bench": "PTH Bench 装配与驱动",
  "src/pth/core": "核心模块（独立维护区）",
  ...
  "src/cli": "pth CLI 入口——runtime 检测/doctor/orchestrator/local-process",
  "src/types": "根级共享类型",
  "packages/pth-contracts": "契约包——类型/结构校验/协议冻结",
  "packages/pth-config": "配置包——PTH_* 键与默认值",
  "packages/pth-memory": "记忆域包——memory 存储/用途层策略/空间可见性/索引/治理执行/skill 格式/只读 SQL",
  ...
  "extensions": "扩展——pth-tasks（任务提交）/agent-lab",
};
...
export const PROJECT_FILE_DUTY: Record<string, string> = {
  ...
  "src/pth/impls/kernels/ts-interpreter.ts": "ts 核（PTC vm——currentCwd 定位任务工作区）",
  "src/pth/impls/kernels/capability.ts": "能力构建（fs/memory/llm 注入）——fs.task 工作区",
