...
const GOLDEN_DESCRIPTIONS: Record<string, string> = {
  ...
  "asp.cd": "【场景锚点：空间切换——我的可达空间内切换注意力】何时用：语言代码须在对应空间执行；done 仅元空间可用；绑定空间仅绑定 worker 类型可进入（语言执行基板全角色共享）。效果：切换当前空间（工具面/记忆域随之切换——注意力重置）。",
...
describe("工具契约注册表（A1 Phase 3 条目 10——ptc/tools 生成器）", () => {
  ...
  it("buildToolSchemas 派生——37 键 + description 与旧手写逐字节一致", () => {
  ...
  it("agent-tools 接线：TOOL_SCHEMAS 已由生成器派生（toolSchemaFor 读同一真相源）", () => {
    ...
    expect(tools).toHaveLength(37);   // 34 + net.search/fetch/extract 3 项
    ...
    expect(tools.some((t) => t.name === "asp_destroy")).toBe(false);
  ...
  it("非 ASP 模式工具面：剔除 ASP-only（schema 与 AGENT_TOOLS 执行面同源——2026-08-15 审计 MEDIUM）", () => {
    ...
    expect(tools).toHaveLength(31);   // 37 - asp.cd/asp.index/memory.index/cache.load/cache.index/cache.cancel
    ...
    // 缺省/ASP 模式仍为全量（向后兼容）
    expect(toolsToSchema()).toHaveLength(37);
    expect(toolsToSchema(undefined, { asp: true })).toHaveLength(37);
  });
});
