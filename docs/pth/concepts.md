...
> **争议标记（2026-08-14）**：〔争议〕= 理论或概念存有未决分歧——已立部分与待议部分分列。当前唯一争议条目：§0.3 智力代偿理论（串联代偿已立 / 并联代偿待议）。
...
| **空间（space）**〔旧〕 | 动作空间的分区容器——**与 worker 类型深度绑定**（2026-08-14 T6 裁决）：新建空间/空间分化属优化行为，不经 worker 直接执行（走优化通道+审批面） | space-registry |
...
### 域 D：进化（理论坐标：0.7 控制论×JIT · 0.3 智力代偿〔争议〕 · 0.10.3 易监督）
...
| N2 | skill 记忆类型（工作流 SOP 一等化） | 0.9 · 0.13 · 域 B | 无实现 | JIT 优化对象（版本化+deopt）· 0.13 外部 skill 映射 | ⚠️ 未实装 |
