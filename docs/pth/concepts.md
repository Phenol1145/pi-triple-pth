...
- **定义**：若某 worker 类型在谱系树中已分拆出子类型，则其动作工具面 = **基本工具 + 面向自己
  直接子类型的任务投递工具**；除这两类外不再提供任何其他工具。
- **基本工具**（所有 worker 通用）：`execTs`（ts 程序面——PTC 唯一通用执行基板）、
  `nav`（空间导航）、`cache`（随身缓存）。
- **投递工具**：rlm 等效任务投递原语（W8 未定项）——只对**直接子类型**开放白名单；
  组织权只决定“可投递范围”，工具面只暴露“下一级”投递点（逐级派发，穿透 skill 是固化捷径）。
- **受影响类型**：谱系内部类型 = actuator / executor / explorer / governor / researcher /
  analyst / developer / tester / prospector（含 controller 系? 否——controller/sensor 是
...
- **设计已完成（2026-08-17 W8）**：入口**强制显式路由**（无缺省入口）；父→子投递用
  ...
  **P1 delegate/await 已实施**：组织权矩阵（直接子类型 + planner/governor 执行族补充权 +
  origin 全树；sensor/controller 排除）服务端 fail-fast；`tasks.delegate/await` 仅投递权角色注入。
...
### 0.17 worker 优化四层次（2026-08-18 用户）

> 给 0.7 优化循环一个**对象维度**：优化作用于 worker 的什么。先钉三个基础概念
> （工具/工具包/可见性），再给四层次。本节是 sensor/controller 按维度细分的概念前提。

#### 0.17.1 工具的形式定义（裁定）

- **工具 = 使用 OpenAI tool call 协议描述的工具**——name / description / parameters
  （JSON schema）的模型函数调用单元。
- 与域 C「工具是双层概念」澄清的关系：双层仍然成立（能力函数 = ts 程序面基板；
  核 = 执行后端）；本节裁定——**优化语境下说「工具」，指 LLM tool call 协议层**
  （schema 面：LLM 可见、可选、可调的那一层）。

#### 0.17.2 工具包（tool pack）

- **具有相同操作对象或相似功能的工具可以放进一个工具包**——包 = 工具的组织单元；
- 包是工具面声明/裁剪的粒度（既有实例：TOOL_GROUPS 的 execTs/dev/debug/write/nav/cache
  族——角色 actionTools 即按包+逐工具 id 混合声明）。

#### 0.17.3 动作空间与记忆空间 = 同一状态的可见性管理

- **动作空间和记忆空间管理的是同一状态下工具和记忆的可见性**：动作空间管「这个
  状态下哪些工具可见/可用」，记忆空间管「这个状态下哪些记忆可见/可取」——
  空间 = 可见性边界（0.10.2 上下文-空间绑定的机制本质；三重交集 = 工具可见性的
  求交规则；memory-visibility = 记忆可见性的过滤规则）。

#### 0.17.4 四层次（优化对象维）

| 层次 | 定义 | 优化产物形态 | 现有机制实例 |
|---|---|---|---|
| **工具面优化** | **提供更多的工具**——减少 LLM 多次调用工具组合出结果的难度（组合成本外移到工具内部） | 新工具/工具包（schema + 执行器） | TOOL_SCHEMAS 扩展 · 穿透 skill（固化调用边——调用组合的固化） |
| **单工具优化** | **扩展单个工具的功能、优化工具描述、优化交互模式** | 工具定义修订（功能/文案/协议） | description 三要素（T8 全量）· 输出模式 mode（回填带宽控制）· 核契约 dispose 语义 |
| **记忆优化** | **优化记忆检索过程、补充不足的信息** | 检索面改进 / 记忆条目补充 | memory.index 两级检索 · skill 渐进披露（W2）· refiner 沉淀（tool-function/task-insight）· 0.15 记忆缺口定位 |
| **规则优化** | **使用文本规则、护栏、权限等措施约束 worker 行为** | 规则文本 / 护栏 / 权限策略 | optimizer-hotspots 规则 stamp · guardrails 注册表（N12）· 用途层权限（memory-policy）· capabilities 白名单 |

#### 0.17.5 与既有理论的咬合

- **与 0.3 代偿**：工具面优化 = 代偿面扩产（一等工具注册通道是并联代偿〔待议〕的
  机制前提）；单工具优化 = 代偿器本身的质量改进；记忆优化 = 命题 2（记忆量分摊）的
  执行侧；规则优化 = 行为约束（0.7 防护环/外环的混合手段）。
- **与 0.7 环**：四层次是 sensor 观测与 controller 调节的**对象维**——每个层次对应
  一类观测信号（工具面缺口/单工具失败模式/记忆缺口/行为越界）与一类调节手段
  （造工具/修工具/补记忆/立规则）。按此四维细分 sensor/controller 并配 SOP = 下一步
  （账本 N14）。
- **与 JIT 产物三态**（2026-08-18 现状盘点）：规则优化已有主环（optimizer-hotspots）；
  记忆优化已有 refiner/两级检索；单工具优化已有 T8 文案与 mode 协议；**工具面优化是
...
> **2026-08-18 用户裁定（0.17.1）**：优化语境下说「工具」= LLM tool call 协议层
> （OpenAI tool call——name/description/parameters）。能力函数是程序面基板，核是执行后端。
...
| **worker 优化四层次**〔新〕 | 优化对象维（0.17.4——2026-08-18 用户）：**工具面优化**（提供更多工具——减组合难度）/ **单工具优化**（扩展功能/优化描述/优化交互模式）/ **记忆优化**（检索过程优化/信息补充）/ **规则优化**（文本规则/护栏/权限约束行为）——sensor/controller 按此四维细分（账本 N14） | optimizer-hotspots（规则）· refiner（记忆）· T8 三要素（单工具）· 穿透 skill（工具面雏形） |
...
| W8 | **PTL 任务派发与交互逻辑（0.16.2 前置设计）**：✅ 已设计（2026-08-17，见 w8-task-dispatch-design.md）——强制显式入口路由；delegate/await 投递原语；事件驱动 requeue 回流；组织权 fail-fast；先契约后收口。**P0–P3 全阶段已实施：契约/盖章/终态回写、组织权矩阵 + delegate/await 工具面、事件回流/取消传播/PTL wait、skill:penetrate:* 接口位（执行优化后续）** | 0.16.2 前置 | ✅ 设计+实施完成（穿透执行优化后续） |
