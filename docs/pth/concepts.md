...
2. **转化**——知识型：SKILL.md → memory 条目（锚点+原文——role-doc 式）；工具型：**拆解源码→用交互核语言重实现**（工具 + **场景化描述三要素**——0.8.2 锚点标准——否则工具不可用）
...
| **场景化描述**〔桥〕 | 工具 description 三要素锚点：【场景锚点】/ 何时用 / 效果预告（0.8.2）——2026-08-14 T8 全量落地：35 个 TOOL_SCHEMAS + 能力索引全分节统一格式 | agent-tools · prompt-docs |
...
| # | 张力 | 双方 | 状态 |
|---|---|---|---|
| T1 | worker-index 全员注入 vs 专注度核心 | 执行类角色收到 22 行无关清单（0.7 核心区寸土寸金 vs 0.8 锚点） | ✅ 已裁决（2026-08-14）：规划系注入+执行族 lazy——isPlanningRole 角色类分化 |
...
| T3 | pick_tools 动态面 vs 缓存命中率 | 工具列表变→前缀变→缓存失效（0.7.2 已承认） | ✅ 已裁决（2026-08-14）：**废弃动态工具注入**——结构化动作空间+记忆空间已减少暴露工具数，pick_tools 退役 |
| T4 | JIT 自动 apply vs 审批面人工闸门 | 0.6.1"控制量不自动生效"vs optimizer-apply 自动 | ✅ 已裁决（2026-08-14）：分层闸门——可逆微调（capability-index/role-doc）自动+deopt；不可逆必人工；PTH_APPLY_POLICY 可配（manual 缺省/auto-reversible） |
...
| T7 | 记忆空间单调增长 vs 有界上下文 | 0.9.4 上下文有界，但记忆库无限增长 | ✅ 已裁决（2026-08-14）：归档闭环补齐执行端——memory-admin 提案批准通道实装（draft→监督批准→archived）；定期触发待 trigger 接线 |
...
| N7 | 记忆归档执行 | 0.8 · 域 B | ✅ 执行端已实装（2026-08-14 T7——memory-admin.ts + 审批路由：draft→监督批准→archived）；定期触发待 trigger 接线 | sensor:memory / controller:memory 已有 | ⚠️ 半实装 |
| N8 | 空间-角色绑定治理 | 0.9.2 · 域 C | T6 裁决定模型：空间与 worker 类型深度绑定，新建/分化走优化通道+审批面（不再设计 TTL 三态）；绑定校验实现待排期 | 治理 v2 承接 | ⚠️ 未实装 |
...
| N10 | agentic 测试集 | 0.5 · 全域 | 规划 v2 在记忆库（official——28 子任务/3 族）；执行 7/28（2026-08-14 消化：4 个 rejected 全部处置——2×T2+v2 规划为迭代性拒绝、重试已成 ✅ 取代；T1/T3/T4 根因=任务文本非自包含规格，重派为 T01/T03/T04 自包含规格 3/3 完成 ✅）；21 子任务未派发 | 测试基线 1503 全绿 | 🏗️ 建设中 |
