...
> 三个子概念：类型树（谱系语义澄清）→ 默认任务拓扑（Origin 入口逐级派发）→ 穿透（固化捷径边）。
...
#### 0.16.2 默认任务拓扑：Origin 默认入口 + 沿类型树逐级派发

- **默认入口 = Origin**：所有执行任务默认以 Origin 为入口（谱系根——全能兜底角色、升级链终点、
  分化演练起点——全新任务领域先由 Origin 承接再观察分化）；
- **逐级派发**：Origin 理解任务 → 沿类型树逐级向下派发（对应类型承接、或继续下派）——
  任务拓扑 = **类型树上的派发路径**，不是运行时长出来的 agent 树；
- **与 Prime 的对照**：Prime 在运行时 spawn 持久子 agent 长成树；PTH 是**静态类型树 + 动态派发路径**——
  「展开成图，而不是长成树」（图的节点 = 类型，边 = 派发/回流）；
- **与现有机制的关系**（待澄清——见 §8.3 W8）：任务正交路由（tags 精确直达）是派发中的直达决策；
  trigger 链（chainDepth）是跨类型的自动续派——两者与「Origin 默认入口」的并存/替代关系待裁决。
...
| 默认入口 Origin | Origin 常驻 + 升级链终点（泛化任务入口语义部分在） | ⚠️ 「所有任务默认 Origin 入口 + 逐级派发」的入口路由待建 |
| 逐级派发 | 正交路由直达 + trigger 链自动续派（chainDepth） | ⚠️ 派发路径显式化（路径记录/审计）待建 |
...
| **默认任务拓扑**〔新〕 | 所有执行任务默认以 **Origin 为入口**，沿类型树逐级派发（2026-08-14 0.16.2）——任务拓扑 = 类型树上的派发路径，不是运行时长出的 agent 树 | role-router（入口路由待建） |
...
| W8 | **默认任务拓扑与正交路由的关系**（2026-08-14 0.16.2 引出）：A 并存——显式路由（tags/flow）直达 + 缺省走 Origin 逐级派发 / B 替换——统一 Origin 入口逐级派发，正交路由退化为派发中的直达决策 / C 其他 | 0.16.2 待澄清点 | ⚠️ 待议 |
