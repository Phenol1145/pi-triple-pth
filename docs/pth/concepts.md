...
| | 旧视角 | 新视角（智力代偿） |
|---|---|---|
| LLM 的角色 | 工具的使用者——任务全程由 LLM 亲自完成，工具只是手 | 工具的制造者——LLM 制造工具，工具**代替它直接完成部分工作** |
...
3. **agentic-JIT（制造持久工具）**：把反复出现的模式固化为规则/技能（optimizer-hotspots 规则 stamp、skill 类型 §10 N2）——后续任务由固化工具直接代偿，LLM 不再重复思考同类问题。**JIT 的 collect→propose→apply→verify 环 = 「LLM 制造工具」的生产线。**
...
| 持久工具（agentic-JIT） | optimizer-hotspots 规则固化 · skill 类型（N2）· 角色分化 |
...
| 智力代偿（0.3） | PTC 程序模式 · agentic-JIT 规则固化 · flash 执行族 |
...
### 域 D：进化（理论坐标：0.7 控制论×JIT · 0.3 智力代偿 · 0.10.3 易监督）
...
| **智力代偿（intelligence compensation）**〔新〕 | 用机械化方法/传统算法/低成本模型代偿 LLM 部分智力负担——工具调用→PTC→agentic-JIT 三级形态（0.3） | PTC 程序 · optimizer 规则固化 · flash 执行族 |
...
12. **工具制造优先**（0.3 智力代偿——能固化的思考固化成工具，不重复思考：PTC 程序 / JIT 规则 / skill 类型）
