...
| 6 | **N25 完整 Human Interaction 协议** | `docs/pth/design/n25-human-interaction-protocol-design.md` | 部分实施（契约 + Intent Resolver + TaskDraft service） | HumanRequest HTTP/持久化/任务 wait gate 已实现；Presentation 等完整协议未实施 | TaskDraft 版本化/质量门/提交证明已落地（内存），PG 持久化待续 |
