...
| 6 | **N25 完整 Human Interaction 协议** | `docs/pth/design/n25-human-interaction-protocol-design.md` | 部分实施（契约 + Intent Resolver + TaskDraft + Presentation + PG TaskDraft Repository 已落地） | HumanRequest HTTP/持久化/任务 wait gate 已实现；PG TaskDraft 持久化适配器已落地但尚未接入 TaskDraftService；完整 UI 未实施 | 核心协议组件已具骨架，PG 持久化待接入服务层 |
...
| 8 | **N28 生产化** | `docs/pth/design/n28-role-memory-orchestration-design.md`、`docs/pth/plan/n28-role-memory-orchestration-implementation-plan.md`、`docs/pth/plan/n28-productionization-plan.md` | 可行性 GO；生产化计划 + M1–M5 骨架 + Repository 缝 + PG 适配器已落地（2026-08-24） | persistent lease/Region 表/outbox 投影/权重校准/重平衡等生产化修缮 | WorkerReplicaLeaseRegistry + MemoryRegionRegistry + CognitiveLedgerOutbox + calibrateWeight + planRebalance + Repository 缝 + PgLeaseRepository/PgRegionRepository/PgRegionMemberRepository/PgCognitiveLedgerOutbox 已落地，注册表接入待续 |
...
| 14 | **仓库结构诊断与重组 P0–P4** | `docs/pth/plan/repo-structure-diagnosis-and-reorganization-plan.md` | P0 文档纠偏 + P1/P2 scripts/test/杂散整理已完成（2026-08-25，v1.8.0 维护批次） | 文档纠偏（architecture/module-ownership/PROJECT_DIR_DUTY）→ 杂散归位 → scripts/test 分区 → src↔packages 收口评审 → 机械化保障 | P0-3/P0-4 最优先已落地；P3 需先出 ADR |
