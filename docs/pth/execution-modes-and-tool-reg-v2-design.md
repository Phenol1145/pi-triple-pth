...
> - `docs/pth/llm-tool-notebook-unified-execution-backend-plan.md`（TCE 三层）
>
...
实施细节另见：[execution-modes-and-tool-reg-v2-implementation-plan.md](./execution-modes-and-tool-reg-v2-implementation-plan.md)。
...
本设计是 `docs/pth/llm-tool-notebook-unified-execution-backend-plan.md` “全量 TCE” 在 tool-reg 上的落地：
...
> 具体任务拆分、文件改动、测试清单、验收标准与风险控制见：[execution-modes-and-tool-reg-v2-implementation-plan.md](./execution-modes-and-tool-reg-v2-implementation-plan.md)。本节只保留高层顺序。
...
- `docs/pth/llm-tool-notebook-unified-execution-backend-plan.md`：TCE 三层 / 全量收编 / 信封模型。
- `docs/pth/n14-sensor-controller-four-dims.md`：tool-reg 一等通道、三态执行体、治理流。
- `docs/pth/concepts.md`：PTC 范式、智力代偿三级形态。
- `docs/pth/task-lifecycle-and-context-design.md`：goal / pause / 压缩 / ASP 双轨。
