...
> 关联：`docs/pth/llm-tool-notebook-unified-execution-backend-plan.md`（TCE 三层——本文档与其正交，接口见 §6）、`packages/pth-contracts/src/tasking.ts`、`packages/pth-kernel-execution/src/execution/context-compaction.ts`、`packages/pth-kernel-execution/src/execution/agent-loop*.ts`、`src/pth/tasking/*`。
