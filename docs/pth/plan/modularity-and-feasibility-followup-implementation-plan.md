...
> 依据：
> - `docs/pth/plan/modularity-reuse-implementation-plan.md`
> - `docs/pth/report/modularity-reuse-audit.md`
> - `docs/pth/report/n28-feasibility-reacceptance-feedback.md`
> - `docs/pth/report/n29-minimal-intake-reacceptance-feedback.md`
> - `docs/pth/plan/n28-role-memory-orchestration-implementation-plan.md`
> - `docs/pth/plan/n29-minimal-knowledge-intake-loop-feedback-plan.md`
...
> 依据 `docs/pth/report/n28-feasibility-reacceptance-feedback.md`。当前已有大部分实现，本阶段是“按反例修复 + 重新验收”，不是从零实现。
...
按 `docs/pth/contract/n28-task7-contract.md` 冻结实现：
...
2. 保存完整 `N28AcceptanceEnvelope`，更新 `docs/pth/report/n28-feasibility-report.md`。
...
> 依据 `docs/pth/report/n29-minimal-intake-reacceptance-feedback.md`。默认 `PTH_KNOWLEDGE_INTAKE_MODE=off` 必须保持到验收通过。
...
2. 保存 `docs/pth/n29-minimal-intake-acceptance.json` 与 `docs/pth/report/n29-minimal-intake-report.md`。
...
> ✅ 已完成（2026-08-23）：`static-all SCC` 从 2 降为 0；`check:import-cycles` 报告 static-runtime/static-all/dynamic 均为 0。
...
> ✅ 已按 D-3 用户选择实施独立 workspace npm 包（2026-08-23）：
> `@away_from/pth-contracts`、`@away_from/pth-config`、`@away_from/pth-kernel-storage`、
> `@away_from/pth-kernel-interpreter`、`@away_from/pth-kernel-execution` 已建立；
> `assembly.ts` 保留在根组合根。ADR 见 `docs/adr/0003-pth-kernel-subpackage-split.md`。
...
> ✅ 维持 Phase A3 决策：跨仓重复脚本采用“同步守卫”，不升级共享脚本包；后续有真实多仓复用需求时再发布 `@away_from/repo-scripts`。
...
1. 更新 `docs/pth/plan/modularity-reuse-implementation-plan.md` 状态表，标注后续计划入口。
2. 更新 `docs/pth/report/modularity-reuse-audit.md` 的遗留项状态。
