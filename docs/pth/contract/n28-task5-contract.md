...
> 对应实施计划 Task 5（docs/pth/plan/n28-role-memory-orchestration-implementation-plan.md L1588–L1983）；验证假设：H5：统一预算是硬上限、确定性、规范化投影计费
...
4. **读取边界冻结**：操作→capability 映射表（7 个 read surface）逐字冻结；family 名不得在读取边界被接受；invalid/expired/missing-capability 时所有 backing port 调用次数为 0。（T7 的 32 个授权探针 = 8 个授权面 × 4 种失效；第 8 授权面 = KnowledgeContext build，见 `docs/pth/contract/n28-lane-contract-rulings.md` 裁决 C2。）
