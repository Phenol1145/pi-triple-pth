...
> 对应实施计划 Task 4（docs/pth/plan/n28-role-memory-orchestration-implementation-plan.md L1107–L1584）；验证假设：H3（错误绑定不造成不可达）+ H4（fallback 授权不变）
...
| Modify: `scripts/tools/n28-feasibility-fixture.ts` | 每行 content 改为包含唯一可检索 token `token:<id>`；新增并冻结 `N28_GOLD_QUERIES` 12 个 gold cases（q-primary-1..4、q-overlap-1..2、q-fallback-1..2、q-global-decoy、q-misbound、q-unclassified-1..2） |
...
- **Step 1**：在 `scripts/tools/n28-feasibility-fixture.ts` 中让每个 fixture 行 content 包含唯一 `token:<id>`，并新增冻结的 `N28_GOLD_QUERIES` 12 个 gold cases。Expected：12 个 gold cases 的 `expectedWave` 分别为 0/0/0/0/1/1/2/2/3/3/2/2，覆盖 primary、overlap、fallback、global decoy、misbound、unclassified。
